...
This function is the **most important** to modify as it is the one that determines the outcome (a vector of length 2) of goals scored for each team. This is the function to modify and where you should add your own model for predicting the outcome of a single match. It can use the `team_data` from above and `team1` and `team2` include vectors of the teams numbers that are playing against each other. `play_game` should return a matrix with dimensions `length(team1)` x `2` with the goals scored by each team.
...
```{r}
result <- simulate_tournament(100)  # Run 100 world cups
...
logloss_score(1:32, prediction_matrix)
